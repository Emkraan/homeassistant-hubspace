...
class EventStream:
    ...
    async def __device_polling(self) -> None:
        ...
        # Emkraan hardening: this loop used to swallow every state-poll
        # failure with just a log line and no event, so a sustained cloud
        # outage on the *state* endpoint (reported directly: intermittent
        # 500s/timeouts on metadevices/.../state) left every device frozen
        # on stale data with zero signal to subscribers — never DISCONNECTED,
        # never RECONNECTED, forever. Track consecutive failures and mirror
        # the discovery loop's own CONNECTED/DISCONNECTED/RECONNECTED
        # signaling so the HA coordinator can actually tell.
        ...
        while True:
            ...
            try:
                ...
                self._logger.warning(
                    "Unable to poll device states (%s consecutive failures)",
                ...
                if (
                    ...
                    self.emit(EventType.DISCONNECTED)
                continue
            if consecutive_state_errors > 0:
                self._logger.info(
                    "Recovered after %s consecutive state-poll failures",
                    consecutive_state_errors,
                )
                consecutive_state_errors = 0
                if self._status != EventStreamStatus.CONNECTED:
                    self._status = EventStreamStatus.CONNECTED
                    self.emit(EventType.RECONNECTED)
