...
class AferoBridgeV1:
    ...
    def controllers_by_name(self) -> dict[str, "BaseResourcesController"]:
        """Get initialized controllers keyed by their bridge attribute name.

        Emkraan addition (not an upstream reliability patch, see NOTICE.md):
        generic HA platforms (sensor/binary_sensor/number/select) need to
        walk every controller type without hardcoding the list, and need the
        attribute name back to re-fetch a live resource via
        ``getattr(bridge, name)`` on each entity update.
        """
        return {
            name: controller
    ...
    async def request(
        ...
        while retries < v1_const.MAX_RETRIES:
            ...
            # Emkraan hardening: a connection-level failure (timeout, reset,
            # DNS hiccup) used to propagate immediately with zero retry and
            # no relation to the status-code backoff below, even though it's
            # the same "Afero cloud is having a bad moment" situation
            ...
            try:
                async with self.create_request(
                    ...
                    if resp.status in (429, 500, 502, 503, 504):
                        self.logger.debug(
                            "Afero API returned %s for %s %s (attempt %s/%s)",
                            resp.status,
                            method,
                            url,
                            retries,
                            v1_const.MAX_RETRIES,
                        )
                        continue
                    # 403 is bad auth
                    if resp.status == 403:
                ...
                self.logger.debug(
                    "Network error contacting Afero API (attempt %s/%s): %s",
                    retries,
                    v1_const.MAX_RETRIES,
                    err,
                )
