...
# Emkraan hardening: treat the token as expired this many seconds before its
# real 118s cutoff, so a request built right at the boundary can't have its
# access token expire server-side mid-flight (in-flight-request race).
TOKEN_REFRESH_MARGIN: Final[int] = 20
...
class AferoAuth:
    ...
    async def is_expired(self) -> bool:
        """Determine if the token is expired (with a safety margin, see TOKEN_REFRESH_MARGIN)."""
        if not self._token_data:
            return True
        return datetime.datetime.now().timestamp() >= (
            self._token_data.expiration - TOKEN_REFRESH_MARGIN
        )
        ...
        async with self._async_lock:
            ...
            if await self.is_expired:
                ...
                try:
                    ...
                    if not retry:
                        raise
                    # Emkraan hardening: the stale refresh token must be
                    # cleared here. Without this, the retry below re-enters
                    # token() with self._token_data still set to the same
                    # dead token, so `is_expired` is still True and it
                    # repeats the identical doomed refresh_token grant
                    # instead of ever falling back to perform_initial_login()
                    # (full username/password login) — the retry was a
                    # no-op that always raised on the second attempt too.
